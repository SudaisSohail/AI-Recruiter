...
def update_store():
    ...
    # zipped_pairs = list(zip(titles, links))
    # zipped_pairs.sort(key=lambda x: x[0])
    # _, links = zip(*zipped_pairs)
    #
    # links = list(links)
...
def run():
    ...
    if len(shortlisted_resumes) != 0:
        ...
        # Remove the header separator row (second row)
        lines = [line for i, line in enumerate(lines) if i != 1]

        # Parse rows and columns by stripping whitespace and splitting by '|'
        table_data = [[col.strip() for col in line.split('|') if col.strip()] for line in lines]
        ...
        st.download_button(
            ...
            data=df.to_csv(),
            file_name=f"shortlisted-{job}.csv",
            mime="text/csv",
        )
    else:
